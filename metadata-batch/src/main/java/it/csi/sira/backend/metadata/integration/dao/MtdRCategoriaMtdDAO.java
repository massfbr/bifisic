/*
 * Created on 18 nov 2016 ( Time 17:08:43 )
 * Generated by Telosys Tools Generator ( version 2.1.1 )
 */
package it.csi.sira.backend.metadata.integration.dao;

import it.csi.sira.backend.metadata.integration.dto.MtdRCategoriaMtd;
import java.util.List;
import java.util.Map;
import org.springframework.jdbc.core.RowMapper;
import org.springframework.jdbc.core.namedparam.MapSqlParameterSource;




/**
 * MtdRCategoriaMtd DAO interface
 * 
 * @author Telosys Tools
 */
public interface MtdRCategoriaMtdDAO {

	//----------------------------------------------------------------------
	/**
	 * Finds a bean by its primary key 
	 * @param idCategoria
	 * @param idMetadato
	 * @return the bean found or null if not found 
	 */
	public MtdRCategoriaMtd findByPK(Integer idCategoria, Integer idMetadato);

	public List<MtdRCategoriaMtd> findAll();
	
	public List<MtdRCategoriaMtd> findByCriteria(java.util.Map<String, Object> params);
	public List<MtdRCategoriaMtd> findByCriteria(MapSqlParameterSource params);
	
	public <V> List<V> findByGenericCriteria(String query, RowMapper<V>  rowMapper, Map<String, Object> params);
	public <V> List<V> findByGenericCriteria(String query, RowMapper<V>  rowMapper, MapSqlParameterSource params);
	public List<MtdRCategoriaMtd> findByGenericCriteria(String query, MapSqlParameterSource params);
	
	public void insert(MtdRCategoriaMtd bean);

	public void update(MtdRCategoriaMtd bean);
	
	public void update(String sql, Map<String, Object> param);

	public int deleteByPK(Integer idCategoria, Integer idMetadato);
	
	public int delete(String query, Map<String, Object> params);
	public int delete(String query, MapSqlParameterSource params);

	public boolean exist(Map<String, Object> params);

    public RowMapper<MtdRCategoriaMtd> getRowMapper();
}
