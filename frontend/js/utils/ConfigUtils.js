    ...
    replaceDeployPlaceHolders(cfg) {
        return Object.keys(cfg).reduce((previous, current) => {
            return assign(previous, {
                [current]: cfg[current] && cfg[current].replace ? cfg[current].replace('${url.geoserver}', 'http://pproo.azo.hr') : cfg[current]
            });
        }, {});
    },
    loadConfiguration: function() {
        return axios.get(localConfigFile).then(response => {
            if (typeof response.data === 'object') {
                defaultConfig = assign({}, defaultConfig, ConfigUtils.replaceDeployPlaceHolders(response.data));
            }
            return defaultConfig;
        });
    },
